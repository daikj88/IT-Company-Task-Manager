...
class PrivateTaskTest(TestCase):
    ...
    def test_detail_task(self):
        res = self.client.get(reverse("task_manager:task-detail", args=[self.task.id]))
        self.assertEqual(res.status_code, 200)
    ...
    def test_update_task(self):
        update_task_type = TaskType.objects.create(name="update_task_type")
        update_user = get_user_model().objects.create_user(
        ...
        response = self.client.post(
            ...
            {
                "name": "updated_task",
                "task_type": update_task_type.id,
                "priority": "Low-Priority",
                "description": "updated_description",
                "deadline": date(2025, 1, 2).strftime("%Y-%m-%d"),
                "is_completed": True,
                "assignees": [self.user.id, update_user.id]
            }
        )
